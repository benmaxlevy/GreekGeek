...
- [x] 4.1 Update signup to create `PENDING` users without issuing session tokens (no org/university fields)
...
- [x] 5.1 Nest `universities` module: ADMIN-only CRUD endpoints
- [x] 5.2 Return 409 when delete blocked by existing organizations

## 6. Organizations API

- [x] 6.1 Nest `organizations` module: ADMIN-only CRUD, list filter by `universityId`
...
- [ ] 11.1 Register → pending message; no authenticated home access
- [ ] 11.2 Pending user login → awaiting-approval screen; cannot reach protected app page
- [ ] 11.3 Admin fill (assign org) + activate → user reaches protected page
