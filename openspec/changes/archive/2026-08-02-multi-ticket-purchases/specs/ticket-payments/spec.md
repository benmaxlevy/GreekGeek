...
**Reason**: Replaced by Purchase model supporting one PaymentIntent for N tickets under a single allocation.
**Migration**: Migrate each TicketPayment row 1:1 to Purchase with `quantity=1`; backfill `Ticket.purchaseId` from the old `ticketId`; derive `eventId`/`allocationId` via ticket→allocation; keep `statusMismatch` and status values; rename enum to `PurchaseStatus`.

### Requirement: Holder-only checkout creates PaymentIntent on platform account

**Reason**: Checkout is now buyer-initiated by `allocationId` + `quantity` with atomic multi-ticket reservation, not per existing unpaid ticket id.
**Migration**: Replace ticket-scoped checkout endpoint and contracts with purchase checkout; reuse PaymentIntent only for open Purchase on same buyer+allocation.
