...
- Platform ADMIN is the full control plane: CRUD universities and organizations, assign/remove memberships, grant/revoke any permission (post-`ACTIVE` only), approve users (`PENDING` → `ACTIVE` with approve-or-deny against requested org/university — **approve** assigns membership to requested org or admin override, **deny** sets `INACTIVE`), reject pending users (`PENDING` → `INACTIVE`), reactivate (`INACTIVE` → `ACTIVE`); ADMIN holds no Membership and bypasses all org-scoped ACL checks
...
- **apps/web**: Admin dashboard routes and pages, updated `/signup` with cascading university/org pickers and pending messaging, awaiting-approval and blocked status routes for authenticated non-`ACTIVE` users; admin pending queue shows requested org (override on approve)
