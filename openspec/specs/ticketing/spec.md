...
Org-scoped event ticketing on existing `Event` records: inline config, per-org and public allocations, ticket issue/void, ADMIN mark-paid escape hatch, guest self-claim, guest list, QR credentials, host-org check-in scanning, Stripe Connect charge-readiness gates for paid sales, and holder embedded Payment Element checkout (see `ticket-payments`).
