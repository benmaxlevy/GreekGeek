...
The system MUST persist events with a string id, required `organizationId` foreign key, required `name` (non-empty string), required `type` (non-empty free-form string — not an enum registry), required `maxHeadcount` (positive integer), optional `location` (nullable string), and timestamps. Events MUST NOT require vendor, budget, ticketing, or location-FK fields in this capability.
