...
(hasDatabase ? describe : describe.skip)(
  'Purchases checkout integration',
  () => {
    ...
    beforeAll(async () => {
      ...
      publicAllocId = Array.isArray(publicAlloc)
        ? publicAlloc[0]!.id
        : publicAlloc.id;
      await tickets.patchTicketing(
        eventId,
    ...
    it('rejects when charges disabled / not on sale / free alloc', async () => {
      ...
      try {
        await expect(
          purchases.checkout(
            { allocationId: publicAllocId, quantity: 1 },
            asUser(buyerAId),
          ),
        ).rejects.toBeInstanceOf(BadRequestException);
      } finally {
    ...
    it('over-request rejected with remaining count; per-user cap enforced', async () => {
      await expect(
        purchases.checkout(
          { allocationId: paidAllocId, quantity: 100 },
          asUser(buyerAId),
        ),
    ...
    it('void unpaid on open purchase cancels whole purchase; void paid leaves totals', async () => {
      ...
      expect(purchase?.status).toBe('canceled');
      expect(
        await prisma.ticket.count({ where: { purchaseId: result.purchaseId } }),
      ).toBe(0);
