...
import { canManageOrgPendingApprovals, destinationForUser } from '@/lib/auth-routing';
    ...
    <AppShell
      ...
      footer={
        <div className="space-y-3">
          <p className="rl-eyebrow px-1">Session</p>
          <p className="truncate px-1 text-xs text-ink-500">{user.email}</p>
          {user.membership?.organizationName ? (
            <p className="truncate px-1 text-xs text-ink-500">{user.membership.organizationName}</p>
          ) : null}
          <Button
            type="button"
            variant="chrome"
